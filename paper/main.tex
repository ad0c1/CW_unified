...
chance and an overlap-only ablation matched the full estimator --- the
separation is carried by the coherence factor alone. End-to-end community recovery showed no improvement at
...
coherence that separates coherent minority-carried structure from
incoherent edges of matched pooled correlation, while remaining a drop-in
weight inside the standard bootEGA pipeline \citep{christensen2021}. The estimator requires no
...
$0.308$ (CW), a difference of $+0.013$. The absolute rates warrant a
...
stability outcomes --- were not executed; the primary condition shows
...
paper ($n=747$ respondents, $77$ binary items, tetrachoric correlations,
EBICglasso candidate graph, $B=1{,}000$ bootstrap resamples), replacing raw
edge weights with Eq.~\eqref{eq:cw} inside an otherwise unchanged bootstrap
EGA pipeline raised the bootstrap probability
