...
recover it. A post-hoc exploratory diagnostic (not preregistered)
characterizes \emph{how} availability is lost: regenerating every
replicate with availability $.5$--$.7$ from its seed shows that in
$97\%$ of the availability-$.6$ replicates all four missing C edges are
incident to a single C item, against $2.4\%$ under a random
missing-edge null. TMFG failure is item-level displacement --- a C item
expelled from the clique loses its within-C edges together --- which
produces the bimodal availability distribution visible in the replicate
counts of Figure~\ref{fig:failuremap} ($.9$ or $\approx.6$, rarely
between). Under the preregistration's locked interpretation branch, this
...
the binding constraint, and carrier-aware edge selection --- at the item
level, where the failure operates --- is the subject of
...
$K_5$ is non-planar). Both pipelines share the same TMFG, so availability
is identical by construction; the two methods are dodged horizontally for
visibility, and the row beneath the axis gives the number of replicates
in each availability cell (the leftmost cells are sparse). Recovery is
near-ceiling at maximal availability and
