...
\caption{Failure map. Exact-C recovery as a function of the number of
true C edges (of ten) retained by the raw-$|\rho|$ TMFG; the structural
maximum is nine because $K_5$ is non-planar. Both pipelines share the same TMFG, so availability
is identical by construction; the two methods are dodged horizontally for
visibility. The lower panel shows how the $1{,}000$ replicates distribute
over availability --- each bar is the denominator of the recovery
proportion plotted above it, and the distribution is bimodal (nine, or
about six when a C item is displaced; retaining exactly two never
occurred). Recovery is
